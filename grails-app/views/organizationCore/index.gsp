		...
		<div id="list-organizationCore" class="content scaffold-list" role="main">
			...
			<thead>
					<tr>
						...
						<g:sortableColumn property="uid" title="${message(code: 'organizationCore.uid.label', default: 'Uid')}" />
					
						<g:sortableColumn property="name" title="${message(code: 'organizationCore.name.label', default: 'Name')}" />
				...
				<g:each in="${organizationCoreInstanceList}" status="i" var="organizationCoreInstance">
					<tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
						...
						<td><g:link action="show" id="${organizationCoreInstance.id}">${fieldValue(bean: organizationCoreInstance, field: "uid")}</g:link></td>
					
						<td>${fieldValue(bean: organizationCoreInstance, field: "name")}</td>
