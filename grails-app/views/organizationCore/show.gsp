...
<title><g:message code="organizationCore.show.label" /></title>
</head>
<body>
	<a href="#show-organizationCore" class="skip" tabindex="-1"><g:message
	...
	<div id="show-organizationCore" class="content scaffold-show"
		role="main">
		<h1>
			<g:message code="organizationCore.show.label" />
		</h1>
			...
			<g:if test="${organizationCoreInstance?.createdBy}">
				<li class="fieldcontain"><span id="createdBy-label"
					...
					class="property-value" aria-labelledby="createdBy-label"><g:fieldValue
							bean="${organizationCoreInstance}" field="createdBy" /></span></li>
			</g:if>
			...
			<g:if test="${organizationCoreInstance?.lastUpdateBy}">
				<li class="fieldcontain"><span id="lastUpdateBy-label"
					...
					class="property-value" aria-labelledby="lastUpdateBy-label"><g:fieldValue
							bean="${organizationCoreInstance}" field="lastUpdateBy" /></span></li>
			</g:if>

			<g:if test="${organizationCoreInstance?.roles}">
				<li class="fieldcontain"><span id="roles-label"
					class="property-label"><g:message
							code="organizationCore.roles.label" default="Roles" /></span> <g:each
						in="${organizationCoreInstance.roles}" var="r">
						<span class="property-value" aria-labelledby="roles-label"><g:link
								controller="organizationRole" action="show" id="${r.id}">
								${r?.encodeAsHTML()}
							</g:link></span>
					</g:each></li>
			</g:if>
		...
		<g:form url="[resource:organizationCoreInstance, action:'delete']"
			method="DELETE">
			<fieldset class="buttons">
				<g:link class="edit" action="edit"
					resource="${organizationCoreInstance}">
